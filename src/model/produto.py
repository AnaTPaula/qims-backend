...
def execute_create_prd(item: dict):
    ...
    database.execute(query=query, params=(item['nome'], item['preco'], item.get('descricao'), item['unidade'],
                                          item['empresaId'], item.get('estoqueMinimo'), item.get('estoqueMaximo'),
                                          item.get('pontoReposicao')))
...
def execute_update_prd(item: dict):
    ...
    database.execute(query=query,
                     params=(item['nome'], item['preco'], item.get('descricao'), item['unidade'],
                             item.get('estoqueMinimo'), item.get('estoqueMaximo'), item.get('pontoReposicao'),
                             item['empresaId'], item['id']))
